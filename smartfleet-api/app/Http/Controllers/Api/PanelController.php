...
class PanelController extends Controller
{
    public function resumen()
    {
        ...
        $cuotasSemana = DB::table('operador_cuota')
            ->where('fecha_inicio', '<=', $hoy)
            ->where('fecha_fin',    '>=', $hoy)
            ->selectRaw('SUM(cuota_objetivo) as objetivo, SUM(cuota_realizada) as realizada')
            ->first();
        ...
        // ── Viajes en proceso (EN_CURSO) ──────────────────────────────────
        $viajesEnCurso = DB::table('viaje as v')
        ...
        $rezagados = DB::table('operador_cuota as oc')
        ...
        // Operadores con cuota < 30% esta semana
        $opsCuotaBaja = DB::table('operador_cuota as oc')
            ->join('operador as op', 'op.id_operador', '=', 'oc.fk_operador')
            ->where('oc.fecha_inicio', '<=', $hoy)
            ->where('oc.fecha_fin',    '>=', $hoy)
            ->where('oc.cuota_objetivo', '>', 0)
        ...
        $viajesPendientes = DB::table('viaje')
        ...
        // Viajes ASIGNADO sin iniciar (llevan más de 2 horas asignados)
        $viajesAsignadosSinIniciar = DB::table('viaje as v')
